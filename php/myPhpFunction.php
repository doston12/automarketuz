...
function connection_to_db($page_name, $query_type)
{
	...
	if ($conn->connect_error) {
		...
		if ($page_name == "cars") {
			...
			$query = "select t.id from company t where t.name like '".$_GET['country']."'";
			$result = $conn->query($query);
			while ($row = $result -> fetch_assoc()) {
				$company_id = $row['id'];
			}
			...
			$query = "insert into user_applied_car(id_company, id_car, f_name, l_name, m_name, passport, passport_copy_path, date) values(".$company_id.",".$car_id.",".$_GET['f_name'].",".$_GET['l_name'].",".$_GET['m_name'].",".$_GET['passport'].",".$_GET['passport_copy'].",".$_GET['date'].")";
			echo "$query";
			$conn->query($query);
			$conn->close();
			// echo "Ok";
